...
function setupEventListeners() {
  ...
  DOM.buttons.restartLevel.onclick = () => { togglePause(false); startGame(); };
  DOM.buttons.newGame.onclick = () => { togglePause(false); showCharacterSelection(); };
...
function createFallingObject() {
  ...
  if (Math.random() < CONFIG.PRIZE_CHANCE) {
    ...
    let randomObstacle;
    do {
      randomObstacle = CONFIG.ARABIC_LETTERS[Math.floor(Math.random() * CONFIG.ARABIC_LETTERS.length)];
    } while (randomObstacle === state.prizeLetter);
...
function updateFallingObjects() {
  ...
  objects.forEach(obj => {
    ...
    if (obj.offsetTop > DOM.gameArea.clientHeight) {
      ...
      if (isColliding(playerRect, objRect)) handleCollision(obj);
    }
  });
}
